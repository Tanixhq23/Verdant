...
function pickFactor(category, label = "") {
  const value = String(label).toLowerCase();
  const categoryMap = FALLBACK_FACTORS[category] || {};
  for (const [key, factor] of Object.entries(categoryMap)) {
    if (value.includes(key)) return factor;
  }
  return 1;
}
...
function buildFallbackSuggestions(category) {
  return [
    {
      title: "Track highest-impact items",
      description:
        `Focus first on the biggest contributors in your ${category} entries, then optimize those before smaller items.`,
    },
    {
      title: "Set a weekly reduction target",
      description:
        "Use a realistic target (for example 10%) and compare week-over-week to maintain consistent progress.",
    },
    {
      title: "Use efficient alternatives",
      description:
        "Replace the most carbon-intensive choices with lower-impact alternatives where practical.",
    },
  ];
}

function getFallbackAnalysis(userData, category) {
  const inputs = Array.isArray(userData?.inputs) ? userData.inputs : [];
  let carbonFootprintKg = 0;
  for (const item of inputs) {
    const name = item?.type || item?.item || item?.name || item?.mode || "";
    const quantity = toNumber(item?.quantity, 1);
    const amount = toNumber(item?.amount, quantity);
    const factor = pickFactor(category, name);
    carbonFootprintKg += amount * factor;
  }

  return {
    carbonFootprintKg: Number(carbonFootprintKg.toFixed(2)),
    suggestions: buildFallbackSuggestions(category),
  };
}
...
${userDataString}
  ...
  try {
    ...
    if (
      ...
      return getFallbackAnalysis(userData, category);
    }
